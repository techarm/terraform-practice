...
variable "instance_class" {
  type        = string
  default     = "db.t3.micro"
  description = "The instance class to create the RBS instance. Requires a free-tier instance class."
...
variable "storage_size" {
  type        = number
  default     = 10
  description = "The amount of storage to allocate for the RDS instance. Should be between 5GB and 10GB."

  validation {
    condition     = var.storage_size >= 5 && var.storage_size <= 10
    error_message = "The storage size must be between 5 and 10 GB."
  }
}
...
variable "engine" {
  ...
  description = "The database engine to use for the RDS instance. Currently only postgres is supported."
...
variable "credentials" {
  ...
  sensitive   = true
  description = "The root username and password for the RDS instance."
