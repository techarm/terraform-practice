...
resource "google_compute_health_check" "mylb" {
  name                = "${local.name}-mylb-myapp-global-health-check"
...
resource "google_compute_backend_service" "mylb" {
  ...
  port_name             = "webserver"
  backend {
    group           = google_compute_region_instance_group_manager.region1_myapp.instance_group
    capacity_scaler = 1
    balancing_mode  = "UTILIZATION"
  }
  backend {
    group           = google_compute_region_instance_group_manager.region2_myapp.instance_group
    capacity_scaler = 1
    balancing_mode  = "UTILIZATION"
  }
}
