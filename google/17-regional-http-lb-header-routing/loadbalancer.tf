...
resource "google_compute_region_backend_service" "myapp" {
...
resource "google_compute_region_url_map" "mylb" {
  ...
  default_service = google_compute_region_backend_service.myapp.self_link
  ...
  path_matcher {
    ...
    default_service = google_compute_region_backend_service.myapp.id

    # App1 - Route based on Header
    route_rules {
      priority = 1
      service  = google_compute_region_backend_service.myapp.id
      match_rules {
        prefix_match = "/"
        ignore_case  = true
        header_matches {
